...
import (
	...
	"github.com/lflxp/lflxp-kubectl/k8s"
	...
	if v == nil || v.Name() == "msg" {
		...
		if len(ttt) > 1 {
			...
			if v, err := g.SetView("msg", maxX*8/100, maxY*8/100, maxX*92/100, maxY*92/100); err != nil {
				...
				if err != nil {
					// panic(err)
					fmt.Fprintln(v, Colorize(err.Error(), "green", "", false, true))
				} else {
					// json格式美化
					fmt.Fprintln(v, podstring)
				}
	...
	if v, err := g.SetView("Pod", 0, 0, origin.maxX-1, origin.maxY-1); err != nil {
		...
		for n, value := range pod_list.Items {
			...
			if len(value.Status.ContainerStatuses) == 0 {
				...
				RestartCount = fmt.Sprintf("%s", fmt.Sprintf("%d", value.Status.ContainerStatuses[0].RestartCount))
			}
			rs := NewCol()
			rs.Data = RestartCount
			rs.TextAlign = TextCenter
			rs.Color = "yellow"
			tableNow.AddRow(4, rs)

			timed := NewCol()
			if value.Status.StartTime != nil {
				timed.Data = fmt.Sprintf("%s", strings.Replace(fmt.Sprintf("%v", value.Status.StartTime.Sub(time.Now())), "-", "", -1))
			} else {
				timed.Data = "none"
			}

			timed.TextAlign = TextLeft
			timed.Color = "yellow"
